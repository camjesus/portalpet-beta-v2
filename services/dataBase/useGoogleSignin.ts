...
import { saveUserAsync } from "../storage/userStorage";
...
export const getGoogleUserInfo = async (token: string | undefined) => {
  try {
    ...
    throw Error("error Reducers: useGoogleSignin: getGoogleUserInfo");
  }
};
...
export const setDefaultUser = async () => {
  try {
    ...
    throw Error("error Reducers: useGoogleSignin: setDefaultUser");
  } 
};
...
export const setDefaultUser2 = async () => {
  try {
    ...
    throw Error("error Reducers: useGoogleSignin: setDefaultUser2");
  } 
};
